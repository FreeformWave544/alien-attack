...
func _ready():
	if not achievements: $CanvasLayer/none.text = "No achievements :(" ; return
...
func achieve(i):
	...
	medal.global_position = Vector2((i * 100) * 2.3, 220.0)
...
func show_tooltip(i, pos):
	...
	achievement.global_position = Vector2(pos.x - (achievement.size.x / 2), pos.y + 100)
...
func hide_tooltip(i):
	var achievement = $CanvasLayer.get_node("ach" + str(i))
	var twin = get_tree().create_tween()
	twin.tween_property(achievement, "modulate", Color(1, 1, 1, 0), 0.3).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT)
	await twin.finished
...
func _on_button_pressed() -> void:
	...
	Global.achievements[Constants.OVERKILL] = true
	_ready()
